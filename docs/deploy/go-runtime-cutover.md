...
This document covers gray release and rollback for Go image runtime. It is an
operator guide only; it does not change production compose defaults.
...
docker compose --profile worker-go up -d worker-go
```
...
The expected operator event text is `nginx reload`. Confirm that route owner
debug headers or service logs show the intended path.

## Rollback

Rollback disables new Go route ownership first:
