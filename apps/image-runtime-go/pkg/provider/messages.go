...
func buildChatMessages(job JobContext, store storage.AssetStorage) ([]map[string]any, error) {
	...
	return buildChatMessagesWithPrompt(job, store, prompt)
}

func buildRawPromptChatMessages(job JobContext, store storage.AssetStorage) ([]map[string]any, error) {
	return buildChatMessagesWithPrompt(job, store, strings.TrimSpace(job.Prompt))
}

func buildChatMessagesWithPrompt(job JobContext, store storage.AssetStorage, prompt string) ([]map[string]any, error) {
