...
def get_site_settings(request: Request, session: Session = Depends(get_db_session)):
    ...
    return api_ok(settings_payload(get_settings_record(session), include_admin_fields=True, session=session))
...
def update_site_settings(
    ...
    session.commit()
    return api_ok(settings_payload(record, include_admin_fields=True, session=session))
