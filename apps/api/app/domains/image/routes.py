...
def create_image_job(payload: CreateImageJobRequest, request: Request, session: Session = Depends(get_db_session)):
    ...
    job = create_job(
        ...
        client_access_id=client_config.client_id if user is None and client_config else None,
...
async def upload_image_asset(
    ...
    asset = persist_uploaded_asset(
        ...
        user_id=user.id if user else None,
        client_id=client_config.client_id if user is None and client_config else None,
    )
    session.commit()
