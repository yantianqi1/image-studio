...
def create_job(
    ...
    *,
    ...
    source_asset = resolve_source_asset(
        session,
        mode=mode,
    ...
    reference_assets = resolve_reference_assets(
        session,
...
def resolve_charge_cents(
    *,
    ...
    if client_provider_config is not None:
        return 0
    return member_price_cents * requested_count if user_id else anonymous_price_cents * requested_count
...
def resolve_source_asset(
    ...
    *,
    ...
    if not asset_access_allowed(asset, user_id=user_id, client_access_id=client_access_id):
...
def asset_access_allowed(asset: Asset, *, user_id: int | None, client_access_id: str | None) -> bool:
    if asset.owner_user_id is not None:
        return asset.owner_user_id == user_id
    if asset.owner_client_id is not None:
        return asset.owner_client_id == client_access_id
    return True
...
def process_render_results(session: Session, *, job: ImageJob) -> None:
    ...
    for result_index in range(1, job.requested_count + 1):
        ...
        rendered = render_job_image(session, render_options=render_options, client_config=client_config)
...
def render_job_image(
    ...
    *,
    ...
    provider_model = str(render_options.get("provider_model") or "")
    source_asset_id = render_options.get("source_asset_id")
    reference_asset_ids = render_options.get("reference_asset_ids")
    return render_image_with_client_provider(
        session,
        config=client_config,
        prompt=str(render_options["prompt"]),
        model_code=str(render_options["model_code"]),
...
def delete_job(
    ...
    *,
    ...
    job = get_job(session, job_id)
    require_job_owner(job, user_id=user_id, client_access_id=client_access_id)
...
def require_job_owner(job: ImageJob, *, user_id: int | None, client_access_id: str | None) -> None:
    if job.user_id == user_id:
        if job.user_id is not None:
            return
        if job.client_access_id is None:
            return
    if job.client_access_id and job.client_access_id == client_access_id:
        return
...
def list_jobs_for_user(
    ...
    *,
    ...
    elif client_access_id is not None:
        statement = statement.where(ImageJob.client_access_id == client_access_id)
    else:
        statement = statement.where(ImageJob.user_id.is_(None), ImageJob.client_access_id.is_(None))
