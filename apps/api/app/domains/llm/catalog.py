...
def ensure_provider_catalog(session: Session) -> None:
    ...
    if official_provider is not None:
        ensure_catalog_model(session, provider=official_provider, seed=build_official_image_model_seed())
    openrouter_provider = ensure_openrouter_provider(session)
    if openrouter_provider is not None:
        ensure_catalog_model(session, provider=openrouter_provider, seed=build_openrouter_image_model_seed())
    session.flush()
