...
class ModelVariant(Base):
    ...
    upstream_cost_credits: Mapped[Optional[float]] = mapped_column(Float, nullable=True)
    upstream_cost_cents: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)
    ...
    anonymous_price_cents: Mapped[int] = mapped_column(Integer, default=0, nullable=False)
    profit_margin_basis_points: Mapped[int] = mapped_column(Integer, default=3000, nullable=False)
    price_manually_set: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)
    status: Mapped[str] = mapped_column(String(32), default="active", nullable=False)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow, nullable=False)


class LLMFeatureModelSetting(Base):
    __tablename__ = "llm_feature_model_settings"

    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    feature_key: Mapped[str] = mapped_column(String(64), unique=True, nullable=False)
    model_code: Mapped[str] = mapped_column(ForeignKey("sellable_models.code"), nullable=False)
