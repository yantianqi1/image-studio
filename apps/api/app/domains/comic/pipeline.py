...
def run_story_analysis(session: Session, *, task: ComicTask, inputs: PipelineInputs) -> ComicStoryAnalysis:
    ...
    payload = call_structured_llm(session=session, schema=StoryAnalysis, schema_name="StoryAnalysis", system_prompt=STORY_ANALYZER_SYSTEM_PROMPT, user_payload={"source_text": inputs.source_text})
...
def run_storyboard(session: Session, *, task: ComicTask, inputs: PipelineInputs, analysis: ComicStoryAnalysis, bible: CharacterBible) -> ComicStoryboard:
    ...
    storyboard = call_structured_llm(session=session, schema=Storyboard, schema_name="Storyboard", system_prompt=STORYBOARD_DIRECTOR_SYSTEM_PROMPT, user_payload=build_storyboard_input(inputs=inputs, analysis=analysis, bible=bible))
...
def call_structured_llm(*, session: Session, schema, schema_name: str, system_prompt: str, user_payload: dict):
    try:
