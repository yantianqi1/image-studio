...
def create_task(client: TestClient) -> dict:
    ...
    response = client.post("/api/public/comic/tasks", json={"project_id": project_id, "chapter_id": "chapter-001", "scene_id": "scene-001", "task_type": "scene-render", "input_payload": build_input_payload()})
    assert response.status_code == 201
    return response.json()["data"]


def create_project(client: TestClient) -> str:
    response = client.post("/api/public/comic/projects", json={"title": "River Blade", "description": "Fixture", "genre": "Wuxia"})
    assert response.status_code == 201
