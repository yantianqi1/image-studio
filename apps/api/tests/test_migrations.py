...
BASELINE_REVISION = "20260426_000005"
...
def test_alembic_upgrade_creates_core_tables(tmp_path):
    ...
    assert {"provider_id", "provider_model", "attempt_count", "max_attempts", "available_at"} <= image_job_columns
    ...
    assert {"stage", "progress_percent", "error_code", "locked_by", "started_at", "finished_at"} <= comic_task_columns
